...
const get = async (req: NextApiRequest, res: NextApiResponse<GET>) => {
  ...
  const treeKeypair = JSON.parse(process.env.TREE??"[]");
  const treeWallet = Keypair.fromSecretKey(
    Uint8Array.from(treeKeypair)
  );
  ...
  console.log("Creator" + ownerWallet.publicKey);
